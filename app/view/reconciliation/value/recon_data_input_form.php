...
<script>
    ...
    function recalc_total() {
        ...
        for (var c=0; c < currency.length; c++){
            ...
            var total_spans = $(currency[c].parentNode).find('span.total');
            ...
            for (var i=0; i<total_trs.length; i++) {
                ...
                for (var j=0; j<recon_summ.length; j++) {
                    ...
                    if (recon_summ[j].value!='') {
                        recon_total += parseInt(recon_summ[j].value)*denom;
                    };
                };
                ...
                if (summa > 0) {
                    $(total_summ[1]).html(parseInt(summa));
                } else {
                    $(total_summ[1]).html('');
                };
            ...
            // Обновляем данные итогов
            $(total_spans[0]).html(sorter_total.toFixed(2));
            $(total_spans[1]).html(recon_total.toFixed(2));
            $(total_spans[2]).html(real_summ.toFixed(2));
        };
    };
