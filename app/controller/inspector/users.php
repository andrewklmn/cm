 ...
 */
        ...
        if ($_POST['order_by']=='RoleLabel') {
            $order = ' ORDER BY '.  addslashes($_POST['order_by']).' '.addslashes($_POST['order_type']);
        } else {
            $order = ' ORDER BY UserConfiguration.'.  addslashes($_POST['order_by']).' '.addslashes($_POST['order_type']);
        };
?>
<div class='container'>
    <h4><?php echo htmlfix($l[1]); ?></h4>
            ...
            <?php
        include 'app/model/table/users_inspector.php';
    ?>
    <br/>
    <input 
        id='0' 
        ...
        ?>
            <script>
                $(document).ready(function () {
                    ...
                    for (var i=1; i<trs.length; i++) {
                        var tds = $(trs[i]).find('td');
                        $(tds[2]).css('background-color','lightgreen');
                    }
                });
            </script>
        <?php        
        ...
        ?>
            <script>
                $(document).ready(function () {
                    ...
                    for (var i=1; i<trs.length; i++) {
                        var tds = $(trs[i]).find('td');
                        $(tds[4]).css('background-color','lightgreen');
                    }
                });
            </script>
        <?php        
