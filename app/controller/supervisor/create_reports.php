 ...
 */
        ...
        $t = explode('|',$_SESSION[$program]['lang']['create_reports_confirmation_header']);
        ...
        if (isset($_POST['action']) AND $_POST['action']==$b[1]) {
            include 'app/controller/common/report/generate_report.php';
        };
        ...
        <?php 
            if (count($xmls)==0 AND count($reports)==0) {
                // Сообщаем что нет данных для отчетов и что дата будет переведена
                        ...
                        $data['info_text'] = '<br/>'.$_SESSION[$program]['lang']['no_data_for_reports'].'<br/>'
                                .$_SESSION[$program]['lang']['start_date_will_be_adjusted'];
                ...
                ?>
                    <h5 style="padding: 0px; margin-bottom:5px;margin-top:25px;">
                        <?php echo htmlfix($_SESSION[$program]['lang']['create_reports_confirm_report_list']); ?>:
                    </h5>
                <?php
            };
        ?>
