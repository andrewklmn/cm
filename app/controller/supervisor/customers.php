 ...
 */
        ...
        $l = explode('|', $_SESSION[$program]['lang']['customers_labels']);
        $b = explode('|', $_SESSION[$program]['lang']['customers_buttons']);
...
<div class='container'>
    <h4><?php echo htmlfix($l[1]); ?></h4>
            <form id='order' method="POST" action="?c=customers" style='margin-bottom: 10px;'>
                ...
                <?php echo htmlfix($l[2]); ?> <input
                                    class='search-query'
                                    style='<?php if (isset($_POST['name']) AND $_POST['name']!='') echo 'background-color:lightgreen;color:darkgreen;'; ?>width:200px;margin: 0px;'
                                    type='text' 
                                    autocomplete="off"
                                    name='name' 
                ...
                <?php echo htmlfix($l[3]); ?> <input 
                                     class='search-query'
                                     type='text' 
                                     style='<?php if (isset($_POST['code']) AND $_POST['code']!='') echo 'background-color:lightgreen;color:darkgreen;'; ?>width:150px;margin: 0px;'
                                     autocomplete="off"
                                     name='code' 
                                     value="<?php if (isset($_POST['code'])) echo $_POST['code']; ?>"/>
                <input class='btn btn-medium' type='submit' value='<?php echo htmlfix($b[0]); ?>' />
                <input class='btn btn-medium' type='submit' name='clear' value='<?php echo htmlfix($b[1]); ?>' />
            </form>
            <?php
        include './app/model/table/customers.php';
    ?>
    <br/>
    <input 
        id='0' 
    ...
    <a class='btn btn-warning btn-large' href='?c=client_add'><?php echo htmlfix($b[3]); ?></a>
</div>
<?php 
