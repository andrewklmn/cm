 ...
 */
    ...
    // Находим все файлы PHP в проекте из папки service (для восстановления)
    exec('cd ..; find . -name "*.php" -not -path "./backups/*" -not -path "./docs/*"', $output);
    // Находим все файлы JS в проекте
    exec('cd ..; find . -name "*.js" -not -path "./backups/*" -not -path "./docs/*"', $output);        
    ...
    exec('cd ..; tar -zcf backups/app.tar.gz app');
    exec('cd ..; tar -zcf backups/bootstrap.tar.gz bootstrap');
    exec('cd ..; tar -zcf backups/Bootstrap_files.tar.gz Bootstrap_files');
    exec('cd ..; tar -zcf backups/js.tar.gz js');
    exec('cd ..; tar -zcf backups/service.tar.gz service');
    exec('cd ..; tar -zcf backups/index.tar.gz index.php');
