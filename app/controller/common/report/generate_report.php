 ...
 */
    ...
    $ticket_xml .= '<CONFIRM xmlns="urn:cbr-ru:csm:v1.0" FilesCount="'.count($ticket_xml_files).'">';
    foreach ($ticket_xml_files as $value) {
        $ticket_xml .= '
    <FileTransfers FileName="'.$value['FileName'].'" CreateTime="'.$value['CreateTime'].'" FileSize="'.$value['FileSize'].'"/>';
    };
    $ticket_xml .= '
</CONFIRM>';
        
    //echo htmlfix($ticket_xml);

    // Стираем всё с прошлого отчета по кассе в АУТПУТ
    exec ('rm -rf '.$xml_output_directory.'/'.$room);
    ...
    // создаем папку с датой в кассе физчески
    /*
    if (!file_exists($xml_output_directory.'/'.$room.'/'.$report_datetime)) {
        mkdir( $xml_output_directory.'/'.$room.'/'.$report_datetime, 0777);
        chmod( $xml_output_directory.'/'.$room.'/'.$report_datetime, 0777);
    };
     * 
     */
    ...
    $fp = fopen( $xml_archive_directory.'/'.$room.'/'.$report_datetime.'/receipt_'.str_replace(' ', 'T', str_replace(':','-',$report_datetime)).'.xml', 'w');
    chmod( $xml_archive_directory.'/'.$room.'/'.$report_datetime.'/receipt_'.str_replace(' ', 'T', str_replace(':','-',$report_datetime)).'.xml', 0777);
    fwrite($fp, $ticket_xml);
    fclose($fp);
